...
async function getTeacherAttachments() {
    try {
        ...
        if (response.ok) {
            ...
            if(submitted['0'].file15_is_checked === true){
...
returnBtn.addEventListener('click', async function(){
    ...
    console.log(content);
    score.value = String(parseInt(score15.value));
    ...
    if (response.ok) {
        console.log("Success Data : ",data);
        setTimeout(function(){window.location.href = "rpms_highlyproficient_tw_plusfactor.html";}, 1000);
    } else {
        console.log("Error Data : ",data);
    }
