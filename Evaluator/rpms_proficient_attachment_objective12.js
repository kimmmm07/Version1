...
window.addEventListener("load", function() {
  const savedValue = localStorage.getItem("obj12");
  if (savedValue) {
  ...
  if (inputElement) {
      inputElement.addEventListener("input", function() {
          ...
          localStorage.setItem("obj12", inputValue);  
      });
  }
});
...
async function getTeacherAttachments() {
    try {
        ...
        if (response.ok) {
            ...
            if(submitted['0'].file12_is_checked === true){
                returnBtn.style.display = 'none';
                score12.value = content['12'].Score;
                score12.disabled = true;  
            }
...
returnBtn.addEventListener('click', async function(){
    ...
    console.log(rpms_id);
    console.log(content);
    if(!parseInt(score12.value) || parseInt(score12.value) > 7){
        alert("Grade should not be 0 and is lower or equal to the max score.");
        return;
    } 
    content['12'].Score = String(score12.value); 
    console.log(content); 
