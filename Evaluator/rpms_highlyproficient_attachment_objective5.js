...
window.addEventListener("load", function() {
  this.alert("asidad");
  const savedValue = localStorage.getItem("obj5_hp");
  if (savedValue) {
  ...
  if (inputElement) {
      inputElement.addEventListener("input", function() {
          ...
          localStorage.setItem("obj5_hp", inputValue);  
      });
  }
});
...
async function getTeacherAttachments() {
    try {
        ...
        if (response.ok) {
            ...
            if(submitted['0'].file5_is_checked === true){
                returnBtn.style.display = 'none';
                score5.value = content['5'].Score;
...
returnBtn.addEventListener('click', async function(){
    ...
    console.log(rpms_id);
    console.log(content);
    if(!parseInt(score5.value) || parseInt(score5.value) > 7){
        alert("Grade should not be 0 and is lower or equal to the max score.");
        return;
    } 
    content['5'].Score = String(score5.value); 
    console.log(content); 
    ...
    if (response.ok) {
        console.log("Success Data : ",data);
        setTimeout(function() {
            window.location.href = 'rpms_highlyproficient_tw_kra2.html';
        }, 1000);
    } else {
        console.log("Error Data : ",data);
    }
