...
window.addEventListener("load", function() {
  this.alert("asidad");
  const savedValue = localStorage.getItem("obj13_hp");
  if (savedValue) {
  ...
  if (inputElement) {
      inputElement.addEventListener("input", function() {
          ...
          localStorage.setItem("obj13_hp", inputValue);  
      });
  }
});
...
async function getTeacherAttachments() {
    try {
        ...
        if (response.ok) {
            ...
            if(submitted['0'].file13_is_checked === true){
                returnBtn.style.display = 'none'; 
                score13.value = content['13'].Score;
                score13.disabled = true;  
            }
